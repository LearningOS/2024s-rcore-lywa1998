...
impl TaskManager {
    ...
    /// asd
    pub fn update_current_task_call(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].call(syscall_id);
    }
